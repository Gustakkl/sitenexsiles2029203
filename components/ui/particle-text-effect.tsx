...
const DEFAULT_WORDS = ["Nexsiles", "Inovação", "Controle", "Semijoias"]
...
export function ParticleTextEffect({ words = DEFAULT_WORDS, className }: ParticleTextEffectProps) {
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    // Adjusted font size for better fit on various screens
    offscreenCtx.font = "bold 130px Manrope, sans-serif" 
    ...
    for (const coordIndex of coordsIndexes) {
      ...
      if (alpha > 0) {
        ...
        if (particleIndex < particles.length) {
          ...
          particle.particleSize = Math.random() * 4 + 2 // Smaller particles for cleaner look
  ...
  const animate = () => {
    ...
    // Background with transparency for blending
